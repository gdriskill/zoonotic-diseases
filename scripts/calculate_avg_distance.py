...
def main(virus_taxid, host_taxid):
    ...
    distances_df = pd.read_csv("../data/hosts/birds/pairwise_distance_avg.csv")
    # Dataframe of virus-host interactions
    interactions_df = pd.read_csv("../data/interactions/NCBI/filtered_virus_host_interactions.csv")

    # Get list of hosts and non-hosts of the virus
    host_with_virus = interactions_df.loc[interactions_df["pathogenID"] == virus_taxid]["hostID"].unique()
    host_without_virus =  interactions_df.loc[interactions_df["pathogenID"] != virus_taxid]["hostID"].unique()
